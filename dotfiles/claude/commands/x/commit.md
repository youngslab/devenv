...
---
⚡ Options:
- "commit" or "Y" → Commit with this message
- "edit: ..." → Modify message and confirm again
- "cancel" → Abort commit
```
...
## Argument Handling
- `$1` (type): feat, fix, docs, etc. → Use as commit type
- `$2` (scope): Use as scope if provided

Examples:
- `/commit` → Auto-analyze
- `/commit fix` → Use fix type
- `/commit feat auth` → feat(auth): ...
